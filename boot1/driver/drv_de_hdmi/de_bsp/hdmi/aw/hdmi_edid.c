...
__s32 DDC_Read(char cmd,char pointer,char offset,int nbyte,char * pbuf)
{
   ...
   *(volatile __u32*)0x1c20150 = 0x80000001;
   //__inf("%x:%x", 0x1c20150, *(volatile __u32*)0x1c20150);
   //__inf("%x:%x", 0x1c20010, *(volatile __u32*)0x1c20010);
...
void GetEDIDData(__u8 block,__u8 *buf)
{
	...
	for (i = 0; i < 8; i++) 
	{
		__inf(" %x  %x  %x  %x  %x  %x  %x  %x  %x  %x  %x  %x  %x  %x  %x  %x\n",
...
__s32 Parse_HDMI_VSDB(__u8 * pbuf,__u8 size)
{
	...
	__inf("3D_multi_present byte(%x,%x)\n",pbuf[index],pbuf[index+1]);

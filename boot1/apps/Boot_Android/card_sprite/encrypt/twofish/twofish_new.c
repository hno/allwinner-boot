...
__u8 tab_5b[4] = { 0, G_M >> 2, G_M >> 1, (G_M >> 1) ^ (G_M >> 2) };
__u8 tab_ef[4] = { 0, (G_M >> 1) ^ (G_M >> 2), G_M >> 1, G_M >> 2 };

#define ffm_01(x)    (x)
#define ffm_5b(x)   ((x) ^ ((x) >> 2) ^ tab_5b[(x) & 3])
#define ffm_ef(x)   ((x) ^ ((x) >> 1) ^ ((x) >> 2) ^ tab_ef[(x) & 3])

__u8 ror4[16] = { 0, 8, 1, 9, 2, 10, 3, 11, 4, 12, 5, 13, 6, 14, 7, 15 };
__u8 ashx[16] = { 0, 9, 2, 11, 4, 13, 6, 15, 8, 1, 10, 3, 12, 5, 14, 7 };
...
__u8
qp(const __u32 n,const __u8 x)
{
	__u8 a0,a1,a2,a3,a4,b0,b1,b2,b3,b4;
...
__u32 h_fun(TWI *pkey,const __u32 x,const __u32 key[])
{
	__u32 b0,b1,b2,b3;
...
void  gen_mk_tab(TWI *pkey,__u32 key[])
{
	__u32 i;
	__u8 by;
...
__u32 mds_rem(__u32 p0,__u32 p1)
{
	__u32 i,t,u;
...
void twofish_new_set_key(TWI *pkey,const __u32 in_key[],const __u32 key_len)
{
	...
	gen_mk_tab(pkey, (__u32 *)pkey->s_key);
}

/* decrypt a block of text  */
/*
void my_i_rnd(TWI * pkey, __u32 * blk , __u32 i)
{
	__u32 t0 = 0;
	__u32 t1 = 0;
	__u32 t2 = 0;
	__u32 i_sum = 0;
    __u32 *key = (__u32 *)pkey->l_key;
	__u32 *mk_tab0 = (__u32 *)&(pkey->mk_tab[0]);
	__u32 *mk_tab1 = (__u32 *)&(pkey->mk_tab[1]);
	__u32 *mk_tab2 = (__u32 *)&(pkey->mk_tab[2]);
	__u32 *mk_tab3 = (__u32 *)&(pkey->mk_tab[3]);
	__u8  *buff = (__u8 *)blk;

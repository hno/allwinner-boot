	  			...
	  			case 0xf3:
	  			{
	  				...
	  				switch(pCBW->CBWCB[1])
	  				{
						...
						case 1:				//小机端接收数据
						{
							...
							start     = (*(int *)(pCBW->CBWCB + 4)) + part_info.addr;
							length    =  write_len;
		  					pusb->device.bo_xfer_addr = pusb->device.bo_memory_base;
		  					pusb->device.bo_xfer_residue = length;
							__inf("write start = %d, size = %d\n", start, length);
							write_start = start;
							fret = epx_out_handler_dev(pusb, pusb->device.bo_ep_out, pusb->device.bo_xfer_addr, pusb->device.bo_xfer_residue, USB_PRTCL_BULK);
							if(fret == USB_RETVAL_COMPOK)
							{
								...
								//flash_ret = wBoot_block_write(start, nsector, (void *)pusb->device.bo_memory_base);
								flash_ret = update_flash_write(start, nsector, (void *)pusb->device.bo_memory_base);
								if(flash_ret < 0)
								{
									__inf("flash write start %d sector %d failed\n", start, nsector);
									pCSW->dCSWSig = USB_CSW_SIG;
						...
						case 3:				//小机端发送数据
						{
							...
							read_len = *(int *)(pCBW->CBWCB + 8);		//扇区数
							start    = *(int *)(pCBW->CBWCB + 4);		//读数据的偏移量
							start   += part_info.addr;

							pusb->device.bo_xfer_addr = pusb->device.bo_memory_base;

							//读出flash数据
							__inf("read start = %d, size = %d\n", start, read_len);
							if(update_flash_read(start, read_len, (void *)pusb->device.bo_memory_base) < 0)
							{
								__inf("read_start = %d, len = %d\n", start, read_len);
								pCSW->dCSWSig = USB_CSW_SIG;
								pCSW->dCSWDataRes = pCBW->dCBWDTL;
								pCSW->bCSWStatus = 1;
								pusb->device.bo_xfer_addr = (__u32)pCSW;
							...
							pusb->device.bo_xfer_residue = min(read_len<<9, MAX_DDMA_SIZE); //Max USB Packet is 64KB    //??

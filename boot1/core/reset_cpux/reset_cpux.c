...
#define AW_CPUCFG_DBGCTL0			(0x1e0)
#define AW_CPUCFG_DBGCTL1			(0x1e4)
...
void cpux_c_start(void)
{
	...
	egon2_writel(CPUX_INVALIDATION_DONE_FLAG, CPUX_INVALIDATION_DONE_FLAG_REG(cpu_id));
	//dsb
	asm volatile ("dsb");
	...
	while(1)
	{
		...
		if(CPUX_INVALIDATION_DONE_FLAG == egon2_readl(CPUX_INVALIDATION_COMPLETION_FLAG_REG)){
			break;
		}
	}
...
__s32 cpucfg_set_cpu_reset_state(__u32 cpu_num, __s32 state)
{
	...
	//set cpu state
	value  = egon2_readl(AW_R_CPUCFG_BASE + CPUX_RESET_CTL(cpu_num));
	value &= ~(CPU_RESET_MASK);
	value |= state;
	egon2_writel(value, AW_R_CPUCFG_BASE + CPUX_RESET_CTL(cpu_num));
...
void super_enable_aw_cpu(int cpu)
{
	...
	//设置每个cpu的地址
	egon2_writel((uint)cpux_s_start, CPUX_RUN_ADDR);
	...
	*/
	...
	egon2_writel(0, (AW_R_CPUCFG_BASE) + CPUX_RESET_CTL(cpu));
	/* L1RSTDISABLE hold low */
	pwr_reg = egon2_readl((AW_R_CPUCFG_BASE) + AW_CPUCFG_GENCTL);
	pwr_reg &= ~(1<<cpu);
	egon2_writel(pwr_reg, (AW_R_CPUCFG_BASE) + AW_CPUCFG_GENCTL);

	/* step2: release power clamp */
	//write bit3, bit4 to 0
	egon2_writel(0xe7, (AW_R_PRCM_BASE) + AW_CPUX_PWR_CLAMP(cpu));
	while((0xe7) != egon2_readl((AW_R_CPUCFG_BASE) + AW_CPUX_PWR_CLAMP_STATUS(cpu)))
	    ;
	//write 012567 bit to 0
	egon2_writel(0x00, (AW_R_PRCM_BASE) + AW_CPUX_PWR_CLAMP(cpu));
	while((0x00) != egon2_readl((AW_R_CPUCFG_BASE) + AW_CPUX_PWR_CLAMP_STATUS(cpu)))
	    ;
	cpux_delay(2);

	/* step3: clear power-off gating */
	pwr_reg = egon2_readl((AW_R_PRCM_BASE) + AW_CPU_PWROFF_REG);
	pwr_reg &= ~(0x00000001<<cpu);
	egon2_writel(pwr_reg, (AW_R_PRCM_BASE) + AW_CPU_PWROFF_REG);
	cpux_delay(1);

	/* step4: de-assert core reset */
	egon2_writel(3, (AW_R_CPUCFG_BASE) + CPUX_RESET_CTL(cpu));
...
int cpu0_set_detected_paras(void)
{
	...
	volatile __u32 value, reg_val;

	//step2: clear completion flag.
	egon2_writel(0, CPUX_INVALIDATION_COMPLETION_FLAG_REG);
	//step3: clear completion done flag for each cpux
	egon2_writel(0, CPUX_INVALIDATION_DONE_FLAG_REG(CPUCFG_CPU1));
	egon2_writel(0, CPUX_INVALIDATION_DONE_FLAG_REG(CPUCFG_CPU2));
	egon2_writel(0, CPUX_INVALIDATION_DONE_FLAG_REG(CPUCFG_CPU3));
	...
	while(1)
	{
		...
		if(CPUX_INVALIDATION_DONE_FLAG == egon2_readl(CPUX_INVALIDATION_DONE_FLAG_REG(CPUCFG_CPU1)) && \
				CPUX_INVALIDATION_DONE_FLAG == egon2_readl(CPUX_INVALIDATION_DONE_FLAG_REG(CPUCFG_CPU2)) && \
				CPUX_INVALIDATION_DONE_FLAG == egon2_readl(CPUX_INVALIDATION_DONE_FLAG_REG(CPUCFG_CPU3)) )
		{
					cpux_uart_putc('i');
					//step9: set completion flag.
					egon2_writel(CPUX_INVALIDATION_DONE_FLAG, CPUX_INVALIDATION_COMPLETION_FLAG_REG);

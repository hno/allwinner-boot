...
unsigned int DRAMC_init(__dram_para_t *para)
{
	...
	*(volatile unsigned int *)(0x01c20000 + 0x160) = 0x81000000U;
	...
	if(((para->dram_tpr13 & (0x3<<3)) == (0x1<<3)) || ((para->dram_tpr13 & (0x3<<3)) == (0x2<<3))) //A31s || A36
	{
	...
	if(para->dram_clk >= 200)
	{
		...
		{
			if((((para->dram_tpr13>>3)&0x3) != 1) && (((para->dram_tpr13>>3)&0x3) != 2))//not A31s && A36
			{
...
//	mctl_reset_release();
	...
	if((para->dram_tpr13 & (0x1u<<31)) == (0x0<<31))
	{
		...
		if(((((para->dram_tpr13>>3)&0x3) == 0) || (((para->dram_tpr13>>3)&0x3) == 3)) && (size_lock == 1))//A31
		{
			if(dram_size != 2048)
			{
		...
		else if((((para->dram_tpr13>>3)&0x3) == 1) || (((para->dram_tpr13>>3)&0x3) == 2))//A31s || A36
		{
